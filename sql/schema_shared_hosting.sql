-- Shared Hosting Import File (phpMyAdmin)
-- Use: pilih database target di phpMyAdmin, lalu import file ini.
-- Catatan: file ini tidak memakai CREATE DATABASE/USE agar kompatibel di shared hosting.
...
-- =========================================================
-- Compatibility layer for newer backend (masterDb.* tables)
-- =========================================================
-- Backend now reads master data from:
--   - teachers (id, name, classification, tmt, is_active)
--   - subjects (id, code, name, is_active)
--   - classes  (id, name)
--   - teacher_tasks
-- These objects map to existing tables (guru, mapel, kelas).

CREATE OR REPLACE VIEW teachers AS
SELECT
  g.guru_id AS id,
  g.nama AS name,
  g.klasifikasi AS classification,
  CASE
    WHEN g.tmt IS NULL OR g.tmt = 0 THEN NULL
    ELSE STR_TO_DATE(CONCAT(g.tmt, '-01-01'), '%Y-%m-%d')
  END AS tmt,
  1 AS is_active
FROM guru g;
...
CREATE TABLE IF NOT EXISTS teacher_tasks (
  ...
  INDEX idx_teacher_tasks_status (status)
);
