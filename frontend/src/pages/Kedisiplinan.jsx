...
export default function Kedisiplinan() {
  ...
  const loadTeachers = async () => {
    ...
    const rows = res.data || [];
    setTeachers(rows);
    setForm((prev) => ({ ...prev, teacherId: prev.teacherId || rows[0]?.id || '0' }));
  };

  const loadSheet = async (targetPeriode) => {
    const p = targetPeriode || periode;
    const res = await api.get('/payroll/discipline/sheet', { params: { periode: p } });
    const rows = res.data || [];
    const nextDrafts = {};
    rows.forEach((row) => {
      nextDrafts[row.id] = {
  ...
  const saveMassal = async () => {
    ...
    await api.put('/payroll/discipline/sheet', { periode, items: payload });
    loadSheet(periode);
  };

  const add = async () => {
    await api.post('/payroll/discipline', {
      tanggal: `${periode}-01`,
    ...
    setShowModal(false);
    setForm((prev) => ({ ...prev, teacherNameManual: '', nominal: 0, keterangan: '' }));
    loadSheet(periode);
  };

  const del = async (id) => {
    await api.delete(`/payroll/discipline/${id}`);
    loadSheet(periode);
  };
  ...
  return (
    <div>
      <div className="modern-table-card">
        <div className="modern-table-title"><Wallet size={24} /> Kedisiplinan</div>
        <div className="toolbar">
          ...
          <input
            ...
            style={{ width: 160 }}
          />
          <button className="outline" onClick={applyMassalToAll}>Terapkan ke Semua</button>
            ...
            {items.map((it, idx) => {
              ...
              return (
                <tr key={it.id}>
                  ...
                  <td>
                    <input type="number" min="0" value={jumlahHadir} onChange={(e) => onDraftChange(it.id, 'jumlahHadir', e.target.value)} style={{ width: 120 }} />
                  </td>
                  <td>
                    <input type="number" min="0" value={nominal} onChange={(e) => onDraftChange(it.id, 'nominal', e.target.value)} style={{ width: 160 }} />
                  </td>
        ...
        {items.length === 0 && <div className="empty">Belum ada data. Tambah Guru terlebih dahulu.</div>}
      </div>
        ...
        <div className="modal-backdrop">
          <div className="modal">
            <div className="modal-header">
              <h3 className="modal-title"><Plus size={24} /> Tambah Guru Kedisiplinan</h3>
